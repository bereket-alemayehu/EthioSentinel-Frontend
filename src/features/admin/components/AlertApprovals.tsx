            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                alerts.map((alert) => {
                  ...
                  return (
                    <tr key={alert.id} className="group hover:bg-slate-50/60 dark:hover:bg-slate-800/60 transition-all cursor-default">
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                          <button
                            ...
                            title="Approve"
                          >
                            {actionLoadingId === String(alert.id) && updateAction === "approve" ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <CheckCircle2 className="w-5 h-5" />}
                          </button>
                          <button
                            type="button"
                            ...
                            title="Reject"
                          >
                            {actionLoadingId === String(alert.id) && updateAction === "reject" ? <div className="w-4 h-4 border-2 border-slate-300 dark:border-slate-500 border-t-slate-600 dark:border-t-slate-100 rounded-full animate-spin" /> : <XCircle className="w-5 h-5" />}
                          </button>
                        </div>
                      </td>
                    </tr>
