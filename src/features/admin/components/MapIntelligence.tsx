...
export function MapIntelligence({
  ...
  const handleExport = async (format: "csv" | "excel" | "pdf") => {
    try {
      ...
      downloadBlob(blob, timestampedFilename(`geo-analytics-${format}`, ext));
    ...
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { icon: AlertTriangle, label: t("highRiskAreasShort"), value: highRiskDistricts, sub: t("thresholdCases"), color: "from-orange-500 to-orange-700" },
          { icon: Activity, label: t("cumulativeCasesShort"), value: totalCases.toLocaleString(), sub: t("fromDataPoints", { count: totalReports }), color: "from-red-600 to-red-800" },
          ...
          <CardHeader className="px-8 py-6 border-b dark:border-slate-800 flex flex-row items-center justify-between bg-white dark:bg-slate-900 relative">
            <div className="space-y-1">
              <CardTitle className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                ...
                {t("diseaseDistributionTitle")}
              </CardTitle>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium italic">{t("diseaseDistributionSubtitle")}</p>
            </div>
            {geoLoading && (
        ...
        <Card className="shadow-2xl shadow-slate-200/40 dark:shadow-none flex flex-col h-[680px] border-none bg-white dark:bg-slate-900">
          <CardHeader className="px-8 py-6 border-b dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
            <div className="flex items-center justify-between mb-1">
              <CardTitle className="text-lg font-black text-slate-800 dark:text-slate-100">{t("hotspotAnalysis")}</CardTitle>
              <Database className="w-5 h-5 text-slate-300 dark:text-slate-600" />
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t("hotspotSubtitle")}</p>
          </CardHeader>
          ...
          <div className="overflow-y-auto flex-1 divide-y divide-slate-100 px-4">
            {geoLoading ? (
              <div className="flex flex-col items-center justify-center gap-4 h-full text-slate-400 dark:text-slate-600">
                ...
                <span className="text-xs font-bold uppercase tracking-widest italic leading-none">{t("scanningDatabases")}</span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-4 h-full opacity-40">
                ...
                <div className="text-xs font-bold uppercase tracking-[0.2em] dark:text-slate-400">{t("zeroHazards")}</div>
              </div>
            ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-3">
                          ...
                          <Badge variant={isHazard ? "destructive" : "success"} className="text-[9px] font-black uppercase px-2 tracking-tighter italic border-none h-5">
                            {isHazard ? t("extremeBadge") : t("monitoredBadge")}
                          </Badge>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                            <div className="flex gap-2">
                              <span>{d.totalCases} {t("casesLabel")}</span>
                              <span className="opacity-30">•</span>
                              <span className={cn(d.totalDeaths > 0 ? "text-red-500 dark:text-red-400" : "")}>{d.totalDeaths} {t("fatalitiesLabel")}</span>
                            </div>
      ...
      <Card className="border-none shadow-2xl p-8 overflow-hidden relative"
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div className="space-y-2 text-white">
            <div className="inline-flex items-center gap-2 bg-white/10 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-2 border border-white/5">{t("aiInsightEngine")}</div>
          ...
          <div className="flex gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="group shrink-0 bg-white dark:bg-slate-900 px-8 py-3.5 h-auto rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-teal-50 dark:hover:bg-slate-800 transition-all shadow-xl hover:-translate-y-1 flex items-center gap-2"
                  style={{ color: "#0f6b7c" }}
                >
